-- MOKSHA — Tables /financer (aides + dossiers)

-- 1. Table des 45 aides publiques
...
-- 2. Table des dossiers de financement
CREATE TABLE IF NOT EXISTS moksha_dossiers_financement (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  aide_id INTEGER NOT NULL REFERENCES moksha_aides(id) ON DELETE CASCADE,
...
CREATE TRIGGER moksha_aides_updated BEFORE UPDATE ON moksha_aides
  FOR EACH ROW EXECUTE FUNCTION moksha_update_timestamp();
...
-- Reset sequence
SELECT setval('moksha_aides_id_seq', 45);
